...
const sumPolysCommand: IWielobotCommand = {
    data: {
        type: 1,
        name: "sum_polys",
        description: "Sums polynomials under given indexes.",
    ...
    async execute(interaction: CommandInteraction, botInstance: Wielobot) {
        ...
        const result = sumPolys(poly1, poly2);
...
function sumPolys(poly1: IPolyTerm[], poly2: IPolyTerm[]): IPolyTerm[] {
    ...
    while(i < poly1.length && j < poly2.length) {
        ...
        if (term1.power === term2.power) {
            const sumCoeff = term1.coeff + term2.coeff;
            if(sumCoeff !== 0) {
                result.push({ power: term1.power, coeff: sumCoeff });
            }
            i++;
            j++;
...
export default sumPolysCommand;